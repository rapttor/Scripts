...
class DataMining
{
    ...
    public static function getUrl($url, $count = 5, $timeout = 10)
    {
        ...
        if (Cache::exists($url)) {
            ...
            if ($httpCode == 301 || $httpCode == 302) {
                ...
                if (isset($matches[1])) {
                    return self::getUrlRecursive(trim($matches[1]), $count + 1);
                }
            } else {
                Cache::set($url, $body);
                return $body;
    ...
    public static function randomUserAgent()
    {
        ...
        foreach ($frequency as $k => $v) $max += $v;
        $roll = rand(0, $max);
        foreach ($frequency as $k => $v) if (($roll <= ($rcount += $v)) and (!$browser_type)) $browser_type = $k;
        $user_agent_array = $browsers[$browser_type];
        shuffle($user_agent_array);

        $agent = $user_agent_array[0];
        return $agent;
    }
}